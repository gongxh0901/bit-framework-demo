 ...
 * @Author: Gongxh
 * @Date: 2026-03-19
 * @Description: 方形碰撞体配置组件，由 ShapeGenerate 系统处理后删除
 */
import { Enum } from "cc";
...
@ecsclass("ShapeBox", { describe: "方形碰撞体配置组件（初始化后删除）" })
export class ShapeBox extends ecs.Component {
    @ecsprop({ type: "float", defaultValue: 0, displayName: "宽度" })
    public width: number = 0;

    @ecsprop({ type: "float", defaultValue: 0, displayName: "高度" })
